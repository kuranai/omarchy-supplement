...
PLYMOUTH_THEME="${PLYMOUTH_THEME:-omarchy}"
...
configure_plymouth_theme() {
  require_command sudo
  require_command plymouth-set-default-theme

  log "Restoring the $PLYMOUTH_THEME Plymouth theme"
...
rebuild_initramfs() {
  ...
  if command -v limine-mkinitcpio >/dev/null 2>&1; then
    log "Rebuilding the Limine unified kernel image"
